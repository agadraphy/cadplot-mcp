...
| Address the intended AutoCAD release | exact bundle route, COM ProgID selection plus version check, safe named-pipe selection, adapter/runtime check, receipt-bound read-only and authenticated publish-session workstation gates | offline route-tamper rejection, Python COM/worker/doctor tests, two-release synthetic read-only/publish-session smoke, prior-evidence tamper/authentication rejection, and current-user named-pipe integration tests; `R25.1`/AutoCAD 2026 is rejected | Implemented locally; exact `20.1`/`25.0` live workstation identities must still be retained in licensed pilot evidence |
| Make the generated local MCP client entry executable without silently invoking CAD work | no-overwrite release-specific `mcpServers` generator, exact command/argument/environment validation, inherited CadPlot/secret/Python environment sanitization, bounded MCP client-config probe | isolated installed-wheel read-only/publish probes, exact 20-tool surface identity, config mutation rejection, zero CadPlot tool calls, workstation evidence and release-manifest binding | Implemented and locally verified; this proves only MCP startup and discovery, while licensed AutoCAD/PDF acceptance remains separate |
...
| Connect an MCP-capable model safely | closed 20-tool STDIO surface, loopback-only HTTP, tunnel preflight, verified-session no-overwrite `mcpServers` config generation, and 13-case tool-selection evaluation | real installed-wheel STDIO/HTTP smokes, read-only/publish config flag separation and overwrite/tamper rejection, header/size guards, canonical evaluation tests | Exact local config generation verified synthetically; licensed session use, Secure MCP Tunnel provisioning, app scan, workspace permissions and live evaluation remain external gates |
...
4. Run a separate small restart/recovery batch on each release and collect both records; validate
   the schema-v8 two-version pilot/recovery document before authorizing a large batch.
