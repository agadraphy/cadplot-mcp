...
| Address the intended AutoCAD release | exact COM ProgID selection plus version check, safe named-pipe selection, adapter/runtime check | Python COM/worker/doctor tests and current-user named-pipe integration tests | Implemented locally; exact `20.1`/`25.0` workstation identities must be retained in pilot evidence |
...
| Install and remove a release safely | isolated clean matching-SDK bundle builder, complete release builder, transactional install/verifiers/uninstallers | stale repository build outputs excluded by construction; synthetic bundle/release/install/uninstall smokes and exact hash-tree verification | Installer path verified synthetically; real matching-SDK bundle and company-machine install remain external |
